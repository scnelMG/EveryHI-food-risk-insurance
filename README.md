...
<p align="center">2023 부산대학교 핀테크융합전공 연구 아이디어 경진대회 대상 · 식단 사진 기록을 건강관리와 보험 탐색의 맥락으로 연결한 인슈어테크 PoC</p>
...
| 구분 | 내용 |
| --- | --- |
| 프로젝트 | 2023 부산대학교 핀테크융합전공 연구 아이디어 경진대회 팀 프로젝트 |
| 수상 | 2023 부산대학교 핀테크융합전공 연구 아이디어 경진대회 대상 |
...
> **분석 흐름**: 식단 사진 → YOLOv5 음식 객체 탐지 → 음식·영양 정보 매핑 → 일·주·월 영양 집계 → 질병 위험 신호 → 보험 보장 항목 규칙 매칭

## 구현과 판단

| 영역 | 구현 | 선택 이유 |
| --- | --- | --- |
| 음식 탐지 | YOLOv5 객체 탐지 | 한 끼 사진에는 여러 음식이 함께 있어 단일 이미지 분류보다 객체 탐지가 맞았습니다. |
...
| 공개 범위 | 원본 데이터·모델 가중치·보험 원문 제외 | 민감 건강 데이터와 재배포 권한이 불명확한 자료를 포트폴리오에서 분리했습니다. |
...
팀 결과와 개인 작업을 구분해, 공개 자료로 확인 가능한 작업만 기록합니다.
...
다른 팀원의 세부 역할은 발표 자료에 근거가 없어 추가하지 않았습니다.
...
> 전체 재학습에는 비공개 원본 음식 이미지·라벨·영양/건강 데이터와 학습 가중치가 필요합니다. 공개 노트북은 입력 계약과 실험 판단을 검토하는 용도입니다.
